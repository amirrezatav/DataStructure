...
namespace DataStructureProject_1
{
    public class ColoredRingStack
    {
        ...
        Table table;
        public ColoredRingStack(int length = 15, int number = 3)
        {
            if (number < 3) throw new Exception("Rods Must Be Greater than 2");
            RodsNum = number;
            this.Length = length;
            Rods = new StackArray<int>[number];
            for (int i = 0; i < number; i++)
                Rods[i] = new StackArray<int>(length);
        ...
        void Print(int step)
        {
            StackArray<int>[] Rodscopy = new StackArray<int>[RodsNum];
            for (int i = 0; i < RodsNum; i++)
                Rodscopy[i] = new StackArray<int>(Length);
            for (int i = 0; i < RodsNum; i++)
                StackArray<int>.Copy(Rodscopy[i], Rods[i]);
