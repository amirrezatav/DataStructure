...
{
    public delegate void MyFunction(Person x);
    ...
    public class Person
    {
        ...
        public Person(MyFunction DispatcherTimer_Tick)
        {
            dispatcherTimer = new DispatcherTimer();
            dispatcherTimer.Interval = Interval;
            dispatcherTimer.Tick += (sender, e) => { 
                
                DispatcherTimer_Tick(this);
            };
        }
    }
    public partial class JulietippusArray
    {
        QueueArray<Person> Corridor;
        QueueArray<Person> GroundFloor;
        QueueArray<Person> Yard;
        ...
        public  JulietippusArray(int JulietippusNumbers , int PrisonCapacity , int CorridorCapacity  ,
        ...
        private void Init()
        {
            Corridor = new QueueArray<Person>((uint)CorridorCapacity + 1);
            GroundFloor = new QueueArray<Person>((uint)PrisonCapacity - (uint)CorridorCapacity + 1);
            Yard = new QueueArray<Person>((uint)PrisonCapacity + 1);
            ...
            lock (Yard)
            {
                try
                {
                    if (!Yard.IsEmpty())
                    {
                        ...
                        QueueArray<Person> temp = new QueueArray<Person>((uint)PrisonCapacity + 1);
        ...
        public void Power_Handle()
        {
            Thread th = new Thread(new ThreadStart(()=> {
                try
                {
                    ...
                    Power = true;
                    MainWindow.pw(false);
                    while (!Corridor.IsFull() && !GroundFloor.IsEmpty())
                    {
                        Person ps = new Person(null);
                        lock(ps)
                        {
